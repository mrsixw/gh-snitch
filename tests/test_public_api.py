...
PACKAGE = Path(__file__).resolve().parent.parent / "src" / "ghsnitch"
...
def test_no_private_names_imported_across_modules(path: Path):
    ...
    for node in ast.walk(_parsed(path)):
        ...
        # spelled `from ghsnitch.x import ...`. Catching only the first would
...
def test_all_entries_exist_and_are_public(path: Path):
    ...
    """
    module = __import__(f"ghsnitch.{path.stem}", fromlist=["__all__"])

...
def test_generate_default_config_contains_last_quarters_comment(tmp_path):
    path = generate_default_config(str(tmp_path / "config.toml"))
    assert "last-quarters" in path.read_text()
...
def test_documented_snake_no_update_check_is_honoured(tmp_path, capsys):
    """The options table named this key `no_update_check` from the day it
    shipped, so configs written to the docs carry the snake spelling. It must
    take effect and say so, rather than being silently ignored."""
    body = "[operatives]\nusers = []\n[updates]\nno_update_check = true\n"
    cfg = config.load_config(str(_write(tmp_path, body)))
    assert cfg["no_update_check"] is True
    err = capsys.readouterr().err
    assert "no_update_check" in err and "no-update-check" in err
